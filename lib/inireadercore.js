...
var skipLineRex = /^\s*(\n|\#|;)/,
     ...
     */
    interPolationRex = /%\(([^)]*)\)/,
    GROUP_NAME_BLACKLIST = ['__proto__'];

function isGroupNameBlacklisted(name) {
    return GROUP_NAME_BLACKLIST.indexOf(name) > -1;
}
...
IniReaderCore.prototype.parseFile = function parseFile() {
    ...
    while (lines.length) {
        ...
        if (!skipLineRex.test(line) && nonWhitespaceRex.test(line)) {
            ...
            if (groupName) {
                if (isGroupNameBlacklisted(groupName)) {
                    this.emit('error', new SyntaxError('Group name not allowed: ' + groupName));
                    return;
                }
...
IniReaderCore.prototype.setParam = function setParam(prop, value) {
    ...
    if (isGroupNameBlacklisted(block)) {
        this.emit('error', new SyntaxError('Group name not allowed: ' + block));
        return;
    }
