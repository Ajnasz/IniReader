 ...
 * @constructor
 */
IniReaderCore.prototype.construct = function (cfg, isAsync) {
...
IniReaderCore.prototype.setParam = function setParam(prop, value) {
    ...
    var blockAndKey = getBlockAndKey(prop, this.values);
    var block = blockAndKey[0];
    var key = blockAndKey[1];
 ...
 */
IniReaderCore.prototype.removeParam = function (prop) {
    if (typeof this.values !== 'object') {
        this.values = Object.create(null);
    }
    var blockAndKey = getBlockAndKey(prop, this.values);
 ...
 * @param {Boolean} [writeHeader] If to write header lines, defaults to true
 */
IniReaderCore.prototype.write = function (file, le, writeHeader) {
    var output = '';
    var fs = require('fs');
    ...
    // create a headline, if not explicitly disabled
    if (writeHeader !== false) {
        var now = new Date();
        output += '; IniReader' + le +
                  '; ' + now.getFullYear() + '-' + (now.getMonth() + 1) + '-' + now.getDate() + le;
    }
